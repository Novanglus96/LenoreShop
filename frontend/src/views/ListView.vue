  ...
  <div class="listpage">
    ...
    <v-dialog v-model="clear_full_dialog" max-width="400">
      <v-card>
        ...
        <v-card-text>
          All {{ fullshoppinglist?.totalitems }} items will be removed, purchased
          or not. The list itself stays.
        </v-card-text>
        <v-card-actions>
          <v-spacer />
          <v-btn variant="text" @click="clear_full_dialog = false">Cancel</v-btn>
          <v-btn
            color="error"
            variant="text"
...
const percentComplete = computed(() =>
  isEmpty.value ? 0 : Math.round((totalPurchased.value / totalItems.value) * 100),
);
